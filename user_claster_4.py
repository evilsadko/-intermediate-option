        ...
        for i in F[h]:
            ...
            for o  in ids_order[int(float(i))]:
                ...
                for i in ids_product_order[OR[0]]:
                    ...
                    all_ += 1
                    try:
                        Sz[product_arr[i,1]][1].append(product_arr[i,2])
                        Sz[product_arr[i,1]][0].append(i)
                    except KeyError:
#                        Sz[product_arr[i,1]] = 1
                        Sz[product_arr[i,1]] = [[i],[product_arr[i,2]]]
        RE[h] = Sz
        print (len(list(Sz.keys())), all_)
    with open("out/claster_sort_user_v1.json", 'w') as js_file:
        json.dump(RE, js_file)      
#---------------------------------------------->    
...
def crt(i):
        ...
        for i in range(len(L1)):
            ...
            Z[L1[i],L2[i]] += L3[i] 
            #print (L1[i],L2[i],L3[i])
        return Z.flatten(), list(res.keys())[0]        
#------------------------------------------------------>
...
def start_similarity():
    ...
    with open(f"out/test_claster_10000_v12x48.json", 'w') as js_file:
        json.dump(G, js_file)  
...
if __name__ == "__main__":
    ...
    #start_similarity()
    #test()
#    Z = json.load(open("out/claster_sort_user_v1.json", 'r'))
#    for K in Z:
#        print (len(list(Z[K].keys())))
    #----------------------------------------->
#Нюансы:
#Самый точная кластеризация для покупателя на векторе 12x48603
#12 - месяца
#48603 - классы продуктов в бд
#Из за такого размера вектора 583236 параметров, так как алгоритм работает по принципу сравнения двух векторов, получаем большую вычислительную сложность, которую нужно оптимизировать.
#Опробовал:
#1 min max кодирование, взял из полученного самый длинный вектор и учитывать его длину(остальные векторы приравнял длине самого длинного вектора, и заполнил нулями ).
#Эксперемент, получил не обьяснимю кластеризацию - не было пересечений продуктов в выявленных группах.
#2 вектор размерности 12x1, 12 - месяца, в каждой позиции месяца было кол во продуктов купленных покумателем, обсалютно неудачный эксперемент
#3 вектора 1x48603 - в таком виде учитывал кол во купленных товаров за год, этот вариант использовал самым первым но получал ошибку, из за задчаи корреляции эта задача стала второстипенной. Для улучшения скорости, оптимезировал сортировку и хранение данных (создания вектора "на лету"), использовал подачу партиями по 10т.
#Для 10т время выполнения 9 часов. Для обьеденения групп, использовать среднее арифметические показатели, сравнение между собой. Ошибку трудно оценить,так для этой задачи
#нет метрики.
...
#Выводы: для решения задачи "Повышения прибыльности за счет продаж", нужны алгоритмы: класстеризация, корреляция, регрессия 
#В класстеризации использован алгоритм косинусного расстояния, так же стоит проверить k-средних (можно заранее определить кол во груп и по ним рассортировывать пользователей), EM-алгоритм, сети Кохонена. По моему мнению: покупательская активность за год предстваленная матрицей, самая точная характеристика покупателя. Данные о покупателе из бд использол для предворительной сортировки.
